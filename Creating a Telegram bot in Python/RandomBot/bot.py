...
def get_coin_side():
    return 'Орёл' if randint(0, 1) == 1 else 'Решка'


coin_inline_keyboard_markup = InlineKeyboardMarkup([[InlineKeyboardButton("Подбросить ещё раз", callback_data='flip_a_coin_again')]])
...
number_inline_keyboard_markup = InlineKeyboardMarkup([[InlineKeyboardButton("Сгенерировать новое", callback_data='new_random_number')]])
...
def main() -> None:
    updater = Updater("5853130478:AAHx2U-54U82INcJVkQPFim3LOyI0K666Qw")
    ...
    updater.dispatcher.add_handler(MessageHandler(Filters.update.message & Filters.text('Подбросить монетку'), flip_a_coin))
    updater.dispatcher.add_handler(MessageHandler(Filters.update.message & Filters.text('Случайное число'), random_number))
